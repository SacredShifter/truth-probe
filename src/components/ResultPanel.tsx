...
import type { ResultPanelProps, AuditResult } from '@/types/audit';
...
export const ResultPanel = ({ result, originalText }: ResultPanelProps) => {
  ...
  const handleExportJSON = () => {
    const exportData = {
      ...
      distortion_flags: result.distortion_flags,
      truth_alignment: result.truth_alignment,
      analysis_report: result.analysis_report,
      valeion_version: "1.0.0"
    };
    ...
    a.href = url;
    a.download = `valeion-audit-${Date.now()}.json`;
  ...
  const handleExportTXT = () => {
    const content = `
VALEION SOVEREIGNTY AUDIT REPORT
====================================
Generated: ${new Date().toLocaleString()}

RESONANCE SCORE: ${result.resonance_score.toFixed(3)} - ${getResonanceLabel(result.resonance_score)}

ORIGINAL TEXT:
${originalText}

DISTORTION FLAGS (${result.distortion_flags.length} detected):
...
====================================
Generated by Valeion Sovereignty Engine v1.0.0
    `;
        ...
        <CardContent className="text-center space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm font-mono">
              <span className="text-muted-foreground">Truth Resonance Level</span>
              <span className={getResonanceColor(result.resonance_score)}>
                {result.resonance_score.toFixed(3)} / 1.000
              </span>
            </div>
            <Progress 
          ...
          <div className={`text-lg font-mono mt-2 ${getResonanceColor(result.resonance_score)} text-center`}>
            {getResonanceLabel(result.resonance_score)}
          </div>
        </CardContent>
      </Card>

      {/* Distortion Flags with Enhanced Styling */}
      {result.distortion_flags.length > 0 && (
        <Card className="bg-background border-border">
          <CardHeader>
            <CardTitle className="text-foreground font-mono flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-red-400" />
              DISTORTION SIGNATURES DETECTED
              <span className="ml-auto bg-red-500/20 text-red-300 px-3 py-1 rounded-md text-sm font-bold">
                {result.distortion_flags.length} FLAG{result.distortion_flags.length !== 1 ? 'S' : ''}
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent>
